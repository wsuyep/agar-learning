...
import time
import timeit
from state import state_function as sf
...
def move_to(id,N,maxN):
    ...
    if(ab.isAlive(id)):
        view = ab.getView(id)
        return view['players'][0]['massTotal'],af.get_states(view,8,10,10)
    else:
        state = []
        return -5000, state 
...
def train_net(model, params):
    ...
    ab.createplayer(botName,botId,50)

    # Create a new game instance.
    first_view = ab.getView(botId)
    game_state = af.get_states(first_view,8,10,10)

    # Get initial state by doing nothing and getting the state.
    first_action = random.randint(0,8)
    _, state = move_to(botId,first_action,8) # move_to will return the new view
    ...
    while t < train_frames:
        ...
        reward, new_state = move_to(botId,action,8)
